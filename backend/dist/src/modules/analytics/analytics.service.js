...
exports.AnalyticsService = {
    async getDashboardMetrics(ctx) {
        const activeMoulds = await prisma_1.prisma.mould.count({ where: { companyId: ctx.companyId, lifecycleState: 'active', deletedAt: null } });
        const pendingEdits = await prisma_1.prisma.editRequest.count({ where: { companyId: ctx.companyId, status: 'pending' } });
        ...
        logs.forEach(l => {
            if (l.downtimeMinutes) {
                ...
                if (l.downtimeReason === 'machine')
                    downtime.machine += l.downtimeMinutes;
                else if (l.downtimeReason === 'mould')
                    downtime.mould += l.downtimeMinutes;
                else if (l.downtimeReason === 'manpower')
                    downtime.manpower += l.downtimeMinutes;
                else
                    downtime.other += l.downtimeMinutes;
            }
        });
    ...
    async getMouldLife(ctx) {
        ...
        const summary = items.reduce((acc, mould) => {
            acc.total += 1;
            acc[mould.riskLevel] += 1;
            acc.byState[mould.lifecycleState] = (acc.byState[mould.lifecycleState] || 0) + 1;
            return acc;
        }, { total: 0, healthy: 0, watch: 0, critical: 0, expired: 0, byState: {} });
        return { summary, items };
    },
    async getDowntime(ctx) {
        const logs = await prisma_1.prisma.dailyProductionLog.findMany({
            where: {
